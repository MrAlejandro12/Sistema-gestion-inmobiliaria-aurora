        ...
        <tr>
          ...
          <td>
            ...
            <?php if (in_array($l['estado'], ['disponible','revision'])): ?>
              <button class="btn btn-red btn-sm" onclick="confirmDelete('lotes.php','¿Eliminar lote <?= addslashes($l['codigo']) ?>?')" data-id="<?= $l['id'] ?>">🗑️</button>
            <?php endif; ?>
          </td>
        </tr>
        <?php endforeach; ?>
...
<div class="modal-bg" id="modal-lote">
  <div class="modal">
    ...
    <form id="form-lote" method="POST" action="lotes.php">
    <?= csrf_field() ?>
